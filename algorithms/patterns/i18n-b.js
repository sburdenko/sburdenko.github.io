...
for (int i = 0; i < n; i++) {
  ...
  'stack.p': {
    en: [
      ...
      { variant: "Monotonic stack: widest rectangle", task: "Bars of width 1. Find the largest rectangle in the histogram.", idea: "Keep indices with increasing heights. A lower bar pops the taller ones: for each popped bar the stack gives the left edge and the current index gives the right edge.", why: "The deepest monotonic-stack problem: “next smaller on both sides” in one pass. Maximal Rectangle (#85) is this, row by row.", cx: "O(n) time · O(n) memory" }
    ],
    ru: [
      ...
      { variant: "Монотонный стек: самый широкий прямоугольник", task: "Столбики ширины 1. Найди самый большой прямоугольник в гистограмме.", idea: "Держим индексы с возрастающими высотами. Более низкий столбик выталкивает высокие: для каждого вытолкнутого левая граница — в стеке, правая — текущий индекс.", why: "Самая глубокая задача на монотонный стек: «ближайший меньший с обеих сторон» за один проход. Maximal Rectangle (#85) — это она же, построчно.", cx: "O(n) время · O(n) память" }
    ],
  },
  'stack.ev.start': {
  ...
  'list.tape': {
    en: '<b>Already on the shelf:</b> fast &amp; slow with both phases of Floyd (the cycle and its entrance) is on <a href="../two-pointers/#floyd">Tape 03 · Two Pointers</a>. Here every problem has its own rig: a fixed gap, rewiring and reversing in groups.',
    ru: '<b>Уже на полке:</b> fast &amp; slow с обеими фазами Floyd (цикл и вход в него) — на <a href="../two-pointers/#floyd">кассете 03 · Two Pointers</a>. Здесь у каждой задачи свой стенд: фиксированный отрыв, перешивка и разворот группами.',
  },
  'list.tpl': {
...
// если цикла нет, slow стоит на середине`,
  ...
  'list.p': {
    en: [
      ...
      { variant: "Reverse in groups", task: "Reverse the list k nodes at a time; a tail shorter than k stays as it is.", idea: "Find the k-th node after groupPrev. If it exists, reverse the group with the usual prev/cur loop and stitch it back: groupPrev → kth, the old first → groupNext.", why: "Combines a dummy node, counting ahead and in-place reversal. The stitching is where people break it — draw it first.", cx: "O(n) time · O(1) memory" }
    ],
    ru: [
      ...
      { variant: "Разворот группами", task: "Разверни список группами по k узлов; хвост короче k оставь как есть.", idea: "Ищем k-й узел после groupPrev. Если он есть, разворачиваем группу обычным циклом prev/cur и пришиваем обратно: groupPrev → kth, старый первый → groupNext.", why: "Собирает вместе dummy-узел, отсчёт вперёд и разворот на месте. Ломаются обычно на пришивании — сначала нарисуй его.", cx: "O(n) время · O(1) память" }
    ],
  },
  'list.ev.start': {
  ...
  'graph.tape': {
    en: '<b>Already on the shelf:</b> queue versus stack, the frontier, the traversal tree and shortest paths are on <a href="../bfs-dfs/">Tape 02 · BFS and DFS</a>. Here every problem has its own rig: flood fill, counting components, multi-source BFS and BFS over words.',
    ru: '<b>Уже на полке:</b> очередь против стека, фронт, дерево обхода и кратчайшие пути — на <a href="../bfs-dfs/">кассете 02 · BFS и DFS</a>. Здесь у каждой задачи свой стенд: заливка, подсчёт компонент, BFS из многих источников и BFS по словам.',
  },
  'graph.tpl': {
...
while (q.Count > 0) {
  ...
  'graph.p': {
    en: [
      ...
      { variant: "BFS over an implicit graph", task: "Turn beginWord into endWord changing one letter at a time; every step must be in the word list. How long is the shortest chain?", idea: "Words are nodes; an edge joins words that differ in one letter. Generate neighbours by trying 26 letters in each position and run BFS level by level.", why: "The graph is never built — neighbours are generated on the fly. The shortest path in an unweighted graph is always BFS.", cx: "O(N · L · 26) time · O(N) memory" }
    ],
    ru: [
      ...
      { variant: "BFS по неявному графу", task: "Преврати beginWord в endWord, меняя по одной букве; каждое слово должно быть в словаре. Какова длина самой короткой цепочки?", idea: "Слова — вершины, ребро соединяет слова с одной отличающейся буквой. Соседей генерируем, перебирая 26 букв в каждой позиции, и идём BFS по уровням.", why: "Граф не строится — соседи рождаются на лету. Кратчайший путь в невзвешенном графе — всегда BFS.", cx: "O(N · L · 26) время · O(N) память" }
    ],
  },
  'graph.ev.start': {
  ...
  'tree.p': {
    en: [
      ...
      { variant: "Return one thing, record another", task: "The maximum sum of a path between any two nodes; the path does not have to pass the root.", idea: "Each node returns its best downward gain (itself + the better child, negatives cut to 0) and, on the side, records the path that turns at it: left + node + right.", why: "The key tree trick: what you return upward differs from what you record globally. Diameter (#543) has the same shape.", cx: "O(n) time · O(h) stack" }
    ],
    ru: [
      ...
      { variant: "Вернуть одно, записать другое", task: "Максимальная сумма пути между любыми двумя узлами; путь не обязан проходить через корень.", idea: "Каждый узел возвращает лучший спуск вниз (сам + лучший ребёнок, отрицательное обрезаем до 0) и попутно записывает путь с поворотом в нём: левый + узел + правый.", why: "Главный приём на деревьях: наверх возвращаешь не то, что записываешь в глобальный ответ. Diameter (#543) устроен так же.", cx: "O(n) время · O(h) стек" }
    ],
  },
  'tree.ev.start': {
